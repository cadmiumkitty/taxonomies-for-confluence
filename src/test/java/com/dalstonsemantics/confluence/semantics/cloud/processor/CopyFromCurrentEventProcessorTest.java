...
public class CopyFromCurrentEventProcessorTest {
    ...
    @Autowired
    private CopyFromCurrentEventProcessor processor;
    ...
    public void shouldCalculateTaxonomyVersionDifference() throws IOException {
        ...
        FileSystemResource taxonomyVersion = new FileSystemResource("./src/test/resources/repositories/client/taxonomy-version-copying-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl");
    ...
    @Test
    @Order(10)
    public void shouldProcessCopyFromCurrentEvent() throws Exception {

        FileSystemResource event = new FileSystemResource("./src/test/resources/events/copy-from-current.ttl");
        Model eventModel = Rio.parse(event.getInputStream(), RDFFormat.TURTLE);

        processor.onEvent(eventModel);

        // Validate that content has been copied by checking the size

        RepositoryConnection connection = taxonomyRepositoryPool.getRepository("taxonomy").getConnection();
        ValueFactory vf = connection.getValueFactory();

        IRI versionOne = vf.createIRI("https://tfc.dalstonsemantics.com/taxonomy/927294f7-0a9f-3d01-8120-b3ca3a45df38-1");

        String expected = Files.readString(Paths.get("./src/test/resources/repositories/client/content-post-copy-from-current-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl"));
        String actual = TestUtils.getRepositoryContent(connection, versionOne);
        assertEquals(expected, actual);

        // Validate content of versions
        ...
        assertEquals(Files.readString(Paths.get("./src/test/resources/responses/processor/CopyFromCurrent.txt")), taxonomyVersion.getBody());
    }
}
