...
public class ClearEventProcessorTest {
    ...
    @Autowired
    private ClearEventProcessor processor;
    ...
    public void shouldCalculateTaxonomyVersionDifference() throws IOException {
        ...
        FileSystemResource taxonomyVersion = new FileSystemResource("./src/test/resources/repositories/client/taxonomy-version-clearing-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl");
    ...
    @Test
    @Order(10)
    public void shouldProcessClearEvent() throws Exception {

        FileSystemResource event = new FileSystemResource("./src/test/resources/events/clear.ttl");
        Model eventModel = Rio.parse(event.getInputStream(), RDFFormat.TURTLE);

        processor.onEvent(eventModel);

        // Validate that content has been cleared by checking the size

        RepositoryConnection connection = taxonomyRepositoryPool.getRepository("taxonomy").getConnection();
        ValueFactory vf = connection.getValueFactory();
        IRI versionOne = vf.createIRI("https://tfc.dalstonsemantics.com/taxonomy/927294f7-0a9f-3d01-8120-b3ca3a45df38-1");

        assertEquals(0, connection.size(versionOne));

        // TODO Validate that event to compute the difference has been fired
        ...
        assertEquals(Files.readString(Paths.get("./src/test/resources/responses/processor/Clear.txt")), taxonomyVersion.getBody());
    }
}
