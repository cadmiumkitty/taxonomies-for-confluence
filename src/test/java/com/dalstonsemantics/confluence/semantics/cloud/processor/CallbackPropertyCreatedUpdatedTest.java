...
public class CallbackPropertyCreatedUpdatedTest {
    ...
    @Autowired
    private CallbackPropertyCreatedUpdatedProcessor processor;
    ...
    @BeforeEach
    public void shouldCalculateTaxonomyVersionDifference() throws Exception {
        TestServicesUtils.resetUUIDProviderSequence(uuidProvider);
    }    

    @Test
    @Order(10)
    public void shouldProcessPropertyCreated() throws Exception {    

        TestServicesUtils.resetPagePropertyToVersionOne(propertyService);
        ...
        FileSystemResource content = new FileSystemResource("./src/test/resources/repositories/client/content-pre-webhook-property-created-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl");
        connection.add(content.getInputStream(), RDFFormat.TURTLE, contentGraph);

        FileSystemResource event = new FileSystemResource("./src/test/resources/events/webhook-property-created.ttl");
        Model eventModel = Rio.parse(event.getInputStream(), RDFFormat.TURTLE);

        processor.onEvent(eventModel);

        String expected = Files.readString(Paths.get("./src/test/resources/repositories/client/content-post-webhook-property-created-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl"));
    ...
    @Test
    @Order(30)
    public void shouldProcessPropertyUpdated() throws Exception {
        ...
        FileSystemResource content = new FileSystemResource("./src/test/resources/repositories/client/content-pre-webhook-property-updated-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl");
        connection.add(content.getInputStream(), RDFFormat.TURTLE, contentGraph);

        FileSystemResource event = new FileSystemResource("./src/test/resources/events/webhook-property-updated.ttl");
        Model eventModel = Rio.parse(event.getInputStream(), RDFFormat.TURTLE);

        processor.onEvent(eventModel);

        String expected = Files.readString(Paths.get("./src/test/resources/repositories/client/content-post-webhook-property-updated-927294f7-0a9f-3d01-8120-b3ca3a45df38.ttl"));
        String actual = TestUtils.getRepositoryContent(connection, contentGraph);
        assertEquals(expected, actual);
    }
}
