...
@Component
@Slf4j
public class ImportResourceEventProcessor extends AbstractTaxonomyEventProcessor {

    private UUIDProvider uuidp;
    private EventDispatcher dispatcher;
    private long taxonomyMaxSize;

    public ImportResourceEventProcessor(
    ...
    protected IRI getEventType() {
        return TEAM.IMPORT_RESOURCE_EVENT;
    }
    ...
    protected void processEvent(RepositoryConnection connection, Model eventModel, 
        ...
        List<Literal> resources = StreamSupport
                .stream(eventModel.getStatements(event, TEAM.RESOURCE, null).spliterator(), false)
        ...
        for (Literal resource : resources) {
            if (connection.size() < this.taxonomyMaxSize) {
                log.info("Importing taxonomy from URI: {}.", resource);
                connection.add(new URL(resource.stringValue()), targetTaxonomyGraph);
            } else {
